...
def search_letter(letter, target):
    for i in letter:
        if i == target:
            return True
    return False
...
def search_event():
    ...
    result = search_letter(list1, target[0])
    entry4.delete(0, END)
    if result:
...
def count_event():
    global list1
    target = str(entry3.get())
    result = count_letter(list1, target[0])
    entry5.delete(0, END)
...
label2 = Label(window, text="Word", fg="black", width=10, font=("arial", 12, "bold"))   #
